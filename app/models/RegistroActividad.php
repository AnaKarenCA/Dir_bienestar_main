...
class RegistroActividad extends Model
{
     ...
     * Esta misma estructura se usa en el helper PermissionHelper.
     */
    ...
    /**
     * Devuelve la jerarquía estática para uso externo (helper).
     * @return array
     */
    public static function getJerarquias()
    {
        return self::JERARQUIAS;
    }

    // ============================================================
    // MÉTODOS CON FILTROS DE PERMISOS
    // ============================================================

    /**
     * Obtiene registros de actividad con filtro de permisos,
     * incluyendo datos de carpeta y tipo de entregable.
     *
     * @param int   $tipoEntregableId
     * @param array $usuario Datos del usuario logueado (con id, rol_id, unidad_administrativa_id)
     * @return array
     */
    public function obtenerRegistrosConPermisos($tipoEntregableId, $usuario)
    {
        $sql = "
            SELECT 
                ra.*,
                ...
                c.fecha_entrega,
                c.firma,
                c.justificacion_fuera_tiempo,
                c.registro_actividad_id
        ...
        ";
        $params = [$tipoEntregableId];

        // Aplicar filtro de permisos (se pasa $this->db)
        $filtro = PermissionHelper::getFiltroSQL($this->db, $usuario, 'ra');
        if ($filtro['sql'] !== '') {
            $sql .= $filtro['sql'];
            $params = array_merge($params, $filtro['params']);
        }

        // Para personal (rol=3), además filtrar por usuario_id
        if ($usuario['rol_id'] == 3) {
     ...
     * @return bool
     */
    public function verificarAcceso($registroId, $usuarioId)
    {
        return PermissionHelper::tieneAccesoARegistro($this->db, $registroId, $usuarioId);
    }
     ...
     * (Método original sin filtrar por permisos, se mantiene para compatibilidad)
     */
    ...
    /**
     * Obtiene un registro completo por su ID (con datos relacionados).
     */
